...
public class ComponentControlWidget : ControlWidget
{
    ...
    public override void OnDragDrop(DragEvent ev)
    {
        if (ev.Data.Object is GameObject go)
        {
            ...
            // Log.Error("DragDrop: " + string.Join(", ", go.Name, c is null, SerializedProperty.Name,
            //     SerializedProperty.PropertyType));

            if (c is not null) return;

            // var subComp = go
            //     .GetComponents().FirstOrDefault(x => x.GetType().IsAssignableTo(SerializedProperty.PropertyType));
            return;
        }
    }
}